...
from src.scheduler.pruning import (
    ...
    build_task_profile,
    dispatch_batch,
    estimate_exec_time_fast,
    estimate_transfer_time_fast,
...
def _best_service_time_fast(task, node_states):
    best = float("inf")
    for ns in node_states:
        st = estimate_transfer_time_fast(task, ns) + estimate_exec_time_fast(task, ns)
        if st < best:
            best = st
    return best


def _min_slack_fast(batch, node_states, now_time):
    best = float("inf")
    for task in batch:
        slack = task["__ddl"] - (now_time - task["__arr"]) - _best_service_time_fast(task, node_states)
        if slack < best:
            best = slack
    return best


def _max_queue_wait(node_states, now_time):
    best = 0.0
    for ns in node_states:
        q = ns["available_time"] - now_time
        if q > best:
            best = q
    return best


def plan_dynamic_window(tasks, nodes, config, use_pruning=False, scheduler_name="dynamic_window"):
    start_alloc_clock = time.perf_counter()

    dw_cfg = config.get("dynamic_window", {})
    pruning_cfg = config.get("pruning", {})

    sort_by = dw_cfg.get("sort_by", "deadline")
    min_batch_size = int(dw_cfg.get("min_batch_size", 2))
    max_batch_size = int(dw_cfg.get("max_batch_size", 6))
    min_wait = float(dw_cfg.get("min_wait", 0.03))
    max_wait = float(dw_cfg.get("max_wait", 0.20))
    slack_trigger = float(dw_cfg.get("slack_trigger", 0.70))
    queue_pressure_trigger = float(dw_cfg.get("queue_pressure_trigger", 0.12))
    ...
    while task_ptr < n or pending:
        if not pending:
            next_task = tasks_sorted[task_ptr]
            current_time = next_task["__arr"]
            pending.append(next_task)
            task_ptr += 1

        batch_start = pending[0]["__arr"]
        forced_dispatch_time = batch_start + max_wait
        dispatched = False

        while not dispatched:
            next_arrival = tasks_sorted[task_ptr]["__arr"] if task_ptr < n else float("inf")

            if next_arrival > forced_dispatch_time:
                result = dispatch_batch(
                    batch=pending,
                    node_states=node_states,
                    decision_time=forced_dispatch_time,
                ...
                pending = []
                current_time = forced_dispatch_time
                dispatched = True
                break

            current_time = next_arrival
            pending.append(tasks_sorted[task_ptr])
            task_ptr += 1
            ...
            if elapsed >= min_wait and len(pending) >= min_batch_size:
                min_slack = _min_slack_fast(pending, node_states, current_time)
                max_queue_wait = _max_queue_wait(node_states, current_time)

                if min_slack <= slack_trigger or max_queue_wait >= queue_pressure_trigger:
                    result = dispatch_batch(
                        batch=pending,
                        node_states=node_states,
                        decision_time=current_time,
...
def run_dynamic_window(tasks, nodes, config, use_pruning=False, scheduler_name="dynamic_window"):
    return plan_dynamic_window(
        tasks=tasks,
        nodes=nodes,
        config=config,
        use_pruning=use_pruning,
